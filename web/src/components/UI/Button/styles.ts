...
export const ButtonBase = styled.button<TButton>`
  ...
  ${({ variant, background }) => variant === 'contained' && {
    backgroundColor: background || 'rgba(0, 0, 0, 0.12)',
    ...containedStyle
  }}

  ${({ variant, color }) => variant === 'outlined' && {
  ...
  ${({ disabled, variant }) => (disabled && variant === 'contained') && {
    backgroundColor: 'rgba(0, 0, 0, 0.12)',
  }}

  ${({ disabled, variant }) => (disabled && variant === 'outlined') && {
